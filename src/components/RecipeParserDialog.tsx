...
interface RecipeParserDialogProps {
  onParsed: (data: {
    ...
    notes: string;
    image?: string;
    screenshotToAdd?: string; // base64 data URL to add as screenshot
  }) => void;
}
...
export function RecipeParserDialog({ onParsed }: RecipeParserDialogProps) {
  ...
  const parseText = async () => {
    ...
    try {
      ...
      setOpen(false);
      toast.success("Рецепт успешно распознан!");
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Ошибка при распознавании");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Распознать рецепт с помощью AI</DialogTitle>
        </DialogHeader>
        ...
        <Tabs defaultValue="photo">
          ...
          <TabsContent value="photo" className="mt-4 space-y-4">
            <p className="text-sm text-muted-foreground">
              AI распознает рецепт из фото. Если на фото нет готового блюда — оно будет добавлено как скриншот.
            </p>
            ...
            >
              {loading ? (
                <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Распознаём...</>
              ) : (
                <><Wand2 className="h-4 w-4 mr-2" />Распознать</>
              )}
            </Button>
          </TabsContent>
          ...
          <TabsContent value="text" className="mt-4 space-y-4">
            <p className="text-sm text-muted-foreground">
              Вставьте текст рецепта — AI заполнит все поля автоматически.
            </p>
            <Textarea
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder="Вставьте текст рецепта сюда..."
              rows={10}
            />
            <Button
              className="w-full"
